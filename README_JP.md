...
* [機能と使い方](#機能と使い方)
* [オプション](#オプション)
* [インストール済のエンジンが表示されない場合](#インストール済のエンジンが表示されない場合)
* [ライセンス](#ライセンス)
* [作者](#作者)
* [開発協力](#開発協力)
* [履歴](#履歴)
...
## インストール済のエンジンが表示されない場合

エンジンインストール時に何らかの原因でWindowsのレジストリにエンジンの情報が登録されない場合に発生します。  
エンジンを再インストールすることで解消しますが、下記の手順でも修正可能です。  

1. レジストリエディタを開く (Windowsのタスクバーの検索ボックスにて「regedit」で検索)
2. 下記のパスのフォルダを開く
   Computer\HKEY_LOCAL_MACHINE\SOFTWARE\EpicGames\Unreal Engine\
3. インストール済みの他のバージョンのフォルダをコピーして登録したいエンジンバージョンにフォルダ名を変更
4. 名前を変更したフォルダ内のInstalledDirectoryプロパティを登録したいエンジンがインストールされているパスに変更
...
- (2025/11/13) v1.8  
  UE5.7に対応しました  
  ビルド構成が正しく読み込まれないバグを修正しました  
  インストールされていないプラットフォームが選択できないように修正しました    
